...
func (c *clientListener) NewBlockAvailable(kapi *SedraApi) {
	c.clientLock.Lock()
		...
		go func(client *gostratum.StratumContext) {
			...
			if err != nil {
				if strings.Contains(err.Error(), "Could not decode address") {
					...
					client.Logger.Error(fmt.Sprintf("failed fetching new block template from sedra, malformed address: %s", err))
					client.Disconnect() // unrecoverable
				} else {
					RecordWorkerError(client.WalletAddr, ErrFailedBlockFetch)
					client.Logger.Error(fmt.Sprintf("failed fetching new block template from sedra: %s", err))
				}
				return
			}
			...
			if !state.initialized {
				...
				state.stratumDiff = newSedraDiff()
	...
	if time.Since(c.lastBalanceCheck) > balanceDelay {
		...
		if len(addresses) > 0 {
			go func() {
				balances, err := kapi.sedrad.GetBalancesByAddresses(addresses)
				if err != nil {
					c.logger.Warn("failed to get balances from sedra, prom stats will be out of date", zap.Error(err))
					return
				}
